...
static int verbs_init_context(ProcessInfo *verbs_processes){
		...
		ctr_info(" > verbs_init_context(): using device %s", ibv_get_device_name(dev_list[0]));

		phot_verbs_context = ibv_open_device(dev_list[0]);
		...
		for (iproc = 0; iproc < _photon_nproc+_photon_forwarder; ++iproc) {
				...
				if( iproc == _photon_myrank ){ continue; }
...
static int verbs_connect_peers(ProcessInfo *verbs_processes){
		...
		for(iproc=0; iproc<_photon_nproc; ++iproc){
				...
				if( !local_info[iproc] ){goto error_exit;}
...
static int verbs_connect_qps(int num_qp, struct verbs_cnct_info *local_info, struct verbs_cnct_info *remote_info, ProcessInfo *verbs_processes){
		...
		for (i = 0; i < num_qp; ++i) {
				/*
						fprintf(stderr,"[%d/%d] iproc=%d, i=%d lid=%x qpn=%x, psn=%x, qp[i].qpn=%x\n",
														_photon_myrank, _photon_nproc, iproc, i,
														remote_info[iproc][i].lid, remote_info[iproc][i].qpn, remote_info[iproc][i].psn,
														verbs_processes[iproc].qp[i]->qp_num);
				 */
				struct ibv_qp_attr attr = {
...
static struct verbs_cnct_info **exch_cnct_info(int num_qp, struct verbs_cnct_info **local_info){
		...
		for (iproc = 0; iproc < _photon_nproc; ++iproc) {
				...
				if (!remote_info){
						for (j = 0; j < iproc; j++){ free(remote_info[j]); }
						free(remote_info);
						goto err_exit;
				}
		}
		...
		for (i = 0; i < num_qp; ++i) {
				for (iproc=0; iproc < _photon_nproc; iproc++){
						...
						if( peer == _photon_myrank ){ continue; }
