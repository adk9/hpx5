...
int resize_requests() {
  ...
  int i;
  for (i = old_capacity; i < rankls->requests_capacity; i++)
...
int get_new_request_num(int* req_num) {
  ...
  // 0 is a null request!
  int i;
  for (i = 1; i < rankls->requests_capacity; ++i) {
...
void mpi_system_register_actions() {
  action_init             = HPX_REGISTER_ACTION(init_action);
  action_shutdown         = HPX_REGISTER_ACTION(shutdown_action);
  action_mark_rank_inited = HPX_REGISTER_ACTION(mark_rank_inited);
  action_send_remote      = HPX_REGISTER_ACTION(send_remote);
  action_scatterv_recv    = HPX_REGISTER_ACTION(scatterv_recv);
...
void mpi_finalize_(int *pier) {
  ...
  int i;
  for (i = 0; i < shared_state->rankls[index].num_comms; i++) {
