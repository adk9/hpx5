...
int pushi(gtc_bench_data_t *gtc_input) {
    ...
    real *temp, *dtemp, *rtemi, *pfluxpsi, *rdtemi, *kapati, *kapan;
...
#if TWO_WEIGHTS
    z6 = particle_data->z6; z06 = particle_data->z06;
#endif
    ...
    kapati = field_data->kapati; kapan = field_data->kapan;
    ...
    real *rmarker, *eflux, *eflux_full, *hfluxpsi, *pmarki;
...
#pragma omp parallel 
{
    real r, rinv, tem, q, qinv, cost, sint, b, g, gp, ri, 
      rip, dbdp, dbdt, dedb, deni, upara, energy, kappa, kappat, kappan, dptdp, dptdt, dptdz, 
      epara, vdr, wdrive, wdrive2, wdrive3, wpara, wdrift, wdot, pdot, tdot, zdot, rdot;
    int ii, ij1, ij2, ij3, ij4, idx1, idx2, idx3, idx4;
    int ip, i;
    real zion0m, zion1m, zion2m, zion3m, zion4m, zion5m, zion6m;
    ...
    real *rmarker_temp = (real *)_mm_malloc((6*mflux+(mpsi+1))*sizeof(real), IDEAL_ALIGNMENT);
    real *eflux_temp = &rmarker_temp[mflux];
    real *eflux_full_temp = &rmarker_temp[2*mflux];
    real *dmark_temp = &rmarker_temp[3*mflux];
    real *dden_temp = &rmarker_temp[4*mflux];
    real *dtem_temp = &rmarker_temp[5*mflux];
    real *hfluxpsi_temp = &rmarker_temp[6*mflux];
...
#pragma omp for schedule(static) nowait
	...
	e3 = 0.0;
	    ...
	    // reflect BC
	    rhotmp = r_diff + rhotmp;
	    if (rhotmp<0.0)
	      rdum = delr * (-rhotmp);
	    else if (rhotmp > a_diff)
	      rdum = delr * (a_diff - (rhotmp - a_diff));
	    else
	      rdum = delr * rhotmp;            
	...
	kappat = wp0 * kapati[ii] + wp1 * kapati[ii+1];
	kappan = wp0 * kapan[ii] + wp1 * kapan[ii+1];

        kappa = ((energy * tem - 1.5) * kappat + kappan) * rinv;
...
#if TWO_WEIGHTS
#if HU_KROMMES
	/* two weights scheme by Hu and Krommes PoP, 1994 */
	wdot = zion6m * (wdrive + wpara + wdrift);
	wdrive3 = q*(-g*dptdp*dbdt + g*dptdt*dbdp - ri*dptdz*dbdp)* deni * zion5m * zion5m * tem;
	/* when paranl==0, the two weights scheme returns to the original formular above */
	//cdot = -((gp*zion3m - 1.0)*paranl*dptdt - paranl*q*(1.0+rip*zion3m)*dptdz)*deni*(upara-dtemp[ii]) * aion * tem * z05[m];
	cdot = -zion6m* (wpara + wdrift - wdrive3);
#else
	/* two weights scheme by W. W. Lee etc. CPC, 2010 */
	wdot = (zion6m - zion4m) * (wdrive + wpara + wdrift);
	wdrive3 = q*(-g*dptdp* dbdt + g*dptdt*dbdp - ri*dptdz*dbdp)* deni * zion5m * zion5m * tem;
	cdot = (zion6m - zion4m) * (wdrive + wdrive3);
#endif

#else // !ONE WEIGHT
	wdot = (1.0 - paranl * zion4m) * (wdrive + wpara + wdrift);
#endif
	...
	if (idiag==0) {
	  ...
	  fullf = z05[m];
#if DELTAF
	  deltaf = fullf*zion04m;
#else
	  deltaf = fullf;
#endif

          vdrenergy = vdr*rinv*(energy-1.5*aion*vthi*vthi*rtemi[ii])*deltaf;
          ...
          rmarker_temp[ip] = rmarker_temp[ip] + fullf; // marker
          eflux_temp[ip] = eflux_temp[ip] + vdrenergy; // energy
          efluxi = efluxi + vdrenergy; // energy
          pfluxi = pfluxi + vdr*rinv*deltaf; // particle
          dflowi = dflowi + b*zion03m*deltaf;
          entropyi = entropyi + deltaf*deltaf;

          particles_energy[0] += energy*deltaf;
          particles_energy[1] += energy;

          dmark_temp[ip] = dmark_temp[ip] + vdr*fullf;
	  dden_temp[ip] = dden_temp[ip] + fullf;
	  sum_of_weights = sum_of_weights + deltaf;
#if TWO_WEIGHTS
	  sum_of_f0 = sum_of_f0 + z06[m];
	  max_of_f0 = max(max_of_f0, z06[m]);
	  min_of_f0 = min(min_of_f0, z06[m]);
#else
          //max_of_f0 = max(max_of_f0, deltaf);
	  max_of_f0 = max(max_of_f0, fabs(wdrive + wdrift));
          //min_of_f0 = min(min_of_f0, deltaf);
	  min_of_f0 = min(min_of_f0, fabs(wpara));
#endif
	}
	...
	if (nonlinear>0.5&&paranl<0.5) {
	  if (istep%ndiag==0) {
	    if (irk==2){
	      ...
	      dden_temp[ip] = dden_temp[ip] + fullf;
	      dtem_temp[ip] = dtem_temp[ip] + energy*deltaf;
	    }
	  }
	}
    } // end m
...
#pragma omp critical 
 {
   if (idiag==0){
     for (i=0; i<mflux; i++) {
       ...
       eflux_full[i] = eflux_full[i] + eflux_full_temp[i];
