...
int gtc_mem_free(gtc_bench_data_t* gtc_input) {
    ...
    _mm_free(diagnosis_data->eflux_full);
...
int setup(gtc_bench_data_t* gtc_input) {
    ...
    real *evector; real *rtemi; real *kapati;
    real *temp, *dtemp;

    real *den00;
    real *difft;
    real *rden, *kapan, *pfluxpsi, *vdrtmp, *hfluxpsi, *zonali, *zonali0, *wtp1, *wtp2;
    ...
    real *scalar_data;
    real *eflux, *eflux_full, *rmarker, *dmark, *dden, *rdtemi, *rdteme;
    real *flux_data;
    ...
    real b, q, dtheta_dx, w_initial, zt1, zt2, z3tmp, wt;
    real kappati, tstep;
    real r, rmax, rmin;
#if USE_MPI
    ...
    tstep = global_params->tstep = 0.015;
...
#if PROFILE_SHAPE == 0
    global_params->background_case = 0;
#elif PROFILE_SHAPE == 1
    global_params->background_case = 1;
#else
    printf("other profile_shape cases are not available\n");
    ...
    global_params->mmomentsoutput = 200;
    if (irun==0) global_params->mstepall = 0;
    global_params->restart_freq = 5000;
    if (global_params->restart_freq<=mstep)
...
#pragma omp barrier
    ...
    tstep = tstep * aion/(fabs(qion)*gyroradius*max(6.0, kappati));
    ...
    kapati   = (real *) _mm_malloc((mpsi+1)*sizeof(real), IDEAL_ALIGNMENT);
    kapan    = (real *) _mm_malloc((mpsi+1)*sizeof(real), IDEAL_ALIGNMENT);
    ...
    zonali0  = (real *) _mm_malloc((mpsi+1)*sizeof(real), IDEAL_ALIGNMENT);
...
#pragma omp parallel for private(i,rad,mthetatmp,mt,q)
    ...
    markerit = (real *) _mm_malloc(nloc_over * mzeta * sizeof(real), IDEAL_ALIGNMENT);
    ...
    densityi0 = (wreal *) _mm_malloc((mzeta+1)*nloc_over*sizeof(wreal), IDEAL_ALIGNMENT);
...
#if TWO_WEIGHTS
    z6       = (real *) _mm_malloc(mimax*sizeof(real), IDEAL_ALIGNMENT);
    z06      = (real *) _mm_malloc(mimax*sizeof(real), IDEAL_ALIGNMENT);
#endif
    ...
    assert(markeri != NULL); assert(markerit != NULL);
    assert(densityi != NULL);
    assert(densityi_local != NULL);
    assert(densityi0 != NULL);
...
#if TWO_WEIGHTS 
    assert(z6 != NULL); assert(z06 != NULL);
#endif
...
#pragma omp parallel for private(i)
    ...
    */
    ...
    eflux_full = (real *) _mm_malloc(mflux*sizeof(real), IDEAL_ALIGNMENT);
    ...
    assert(eflux!=NULL); assert(eflux_full != NULL); assert(rmarker!=NULL); assert(dmark!=NULL);assert(dden!=NULL);assert(rdtemi!=NULL); assert(rdteme!=NULL);
...
#pragma omp parallel for private(i)
...
#if TWO_WEIGHTS 
    int nparam = 14; // modified by bwang May 2013
#else
    int nparam = 12;
#endif
    recvbuf_size = (mi/5);
    ...
    diagnosis_data->eflux_full = eflux_full;
...
#if TWO_WEIGHTS
    particle_data->z6 = z6; particle_data->z06 = z06;
#endif
