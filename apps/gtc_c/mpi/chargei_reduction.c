...
int chargei(gtc_bench_data_t *gtc_input) {
    ...
    wreal *recvr; real *zonali; real* zonali0; real *markeri; real *markerit; real *pmarki; real *rden;
    int *recvr_index;
    ...
    densityi = field_data->densityi; densityi0 = field_data->densityi0;
    ...
    pmarki = field_data->pmarki; rden = field_data->rden;
    markerit = field_data->markerit; zonali0 = field_data->zonali0;
...
#pragma omp for
	...
	assert(ip<=ipsi_valid_out);
	  ...
	  /* accumulate the charge to the boundary if the original position is out of the domain */
	  //rdum = delr * abs_min_real(a_diff, r_diff+rhotmp);
	  /* reflect boundary condition: reflect the charge around the boundary */
...
#pragma omp parallel for 
    for (int i=ipsi_in; i<ipsi_out+1; i++) {
		...
		real den = 0.25*densityi[k+ij*(mzeta+1)];
#if !DELTAF
#if FULL_F_TYPE == 1
		den = 0.25*(densityi[k+ij*(mzeta+1)]-densityi0[k+ij*(mzeta+1)]);
#endif
#endif
                if (i>=ipsi_nover_in&&i<=ipsi_nover_out)
                  zonali[i] += den;
                densityi[k+ij*(mzeta+1)] = den*markeri[k-1+ij*mzeta];
		// time-average # of marker particles per cell for full-f simulation
		if (irk==2)
		  markerit[k-1+ij*mzeta] += den;
            }
        }
    }
...
#if !DELTAF 
#if FULL_F_TYPE == 0
    /* full-f: zonal flow substracted by equilibrium density */
#pragma omp parallel for
    for (int i=0; i<mpsi+1; i++){
    ...
    /* Zero out charge in radial boundary cell */
    for (int i=0; i<nbound; i++){
      zonali[i] = zonali[i]*(real)i/(real)nbound;
      zonali[mpsi-i] = zonali[mpsi-i]*(real)i/(real)nbound;
    }
...
#pragma omp parallel for
    ...
    */
    ...
    if (irk==2&&istep==params->mstep){
#pragma omp parallel for
      for (int i=0; i<mzeta*nloc_over; i++){
	markerit[i] = markerit[i]/(real)(params->mstep);
      }
    }
...
int chargei_init(gtc_bench_data_t* gtc_input) {
    ...
    real* zion2; real* zion5;
    real* zion05; real* zion4;

    int* kzion; real* wzion; 

    int* jtion0; int* jtion1; real* wtion0;
    real* wtion1; real* wpion; 
    const int* igrid; const real* delt; const real* qtinv;
    const int* mtheta; 
    int mi; 
    real smu_inv; real a0;
    real a1; real delr; real delz; real deltar;
    int mpsi; real rho_max;
    int ipsi_in, ipsi_out, ipsi_nover_in, ipsi_nover_out, ipsi_valid_in, ipsi_valid_out, igrid_in, nloc_over;
    real pi2_inv; real zetamin; 
    int mzeta;
    real *pgyro, *tgyro;
    real *zonali0;
    real *pmarki;
    real *densityi, *densityi0;
#if USE_MPI
    real *sendl, *recvr;
    real *dnitmp;
    int *recvr_index;
    ...
    int iptmp, ip, jttmp, jt, ipjt, kk, ii, im, im2, m, larmor, idx, idx1, ij1, ij2;
    real rdum, tflr;
    int j01, j00, jtion0tmp, jtion1tmp,  mpsi_max;
    real tdumtmp, tdum, tdum2, tdumtmp2, wtion0tmp, wtion1tmp;
    real r_diff, a_diff;
    real weight;
    real wz0, wz1, wt00, wt01, wt10, wt11, wp0, wp1;
    ...
    zonali0 = grid->zonali0;
    pmarki = grid->pmarki;
    densityi = grid->densityi;
    densityi0 = grid->densityi0;
#if USE_MPI
    recvr = grid->recvr;
    recvr_index = grid->recvr_index;
    sendl = grid->sendl;
    dnitmp = grid->dnitmp;
#endif

    zion0 = zion->z0; zion1 = zion->z1; zion2 = zion->z2;
    zion5 = zion->z5; zion4 = zion->z4; zion05 = zion->z05; 
	...
	assert(ip<=ipsi_valid_out);
	    ...
	    wp0     = 1.0 - wp1;
            ...
            densityi[ij1] += wz0*wt00;
            densityi[ij1+1] += wz1*wt00;
            densityi[ij1+mzeta+1] += wz0*wt10;
            densityi[ij1+mzeta+2] += wz1*wt10;

            densityi[ij2] += wz0*wt01;
            densityi[ij2+1] += wz1*wt01;
            densityi[ij2+mzeta+1] += wz0*wt11;
            densityi[ij2+mzeta+2] += wz1*wt11;
#endif
        }
    }
...
#pragma omp parallel for
    for (int j=0; j<nloc_over; j++){
      for (int k=1; k<mzeta+1; k++){
        densityi0[(mzeta+1)*j+k] = densityi[(mzeta+1)*j+k];
      }
    }
    ...
    for (int i=ipsi_in; i<ipsi_out+1; i++) {
      for (int j=1; j<mtheta[i]+1; j++) {
        for (int k=1; k<mzeta+1; k++) {
          int ij = igrid[i] + j - igrid_in;
          if (i>=ipsi_nover_in&&i<=ipsi_nover_out)
            zonali0[i] += 0.25*densityi0[k+ij*(mzeta+1)];
        }
      }
    }

#if USE_MPI
    real *adum = (real *) _mm_malloc((mpsi+1)*sizeof(real), IDEAL_ALIGNMENT);
    real *adum2 = (real *) _mm_malloc((mpsi+1)*sizeof(real), IDEAL_ALIGNMENT);
    MPI_Allreduce(zonali0, adum2, mpsi+1, MPI_MYREAL, MPI_SUM,
                  parallel_decomp->toroidal_comm);
    MPI_Allreduce(adum2, adum, mpsi+1, MPI_MYREAL, MPI_SUM,
                  parallel_decomp->partd_comm);
#pragma omp parallel for
    for (int i=0; i<mpsi+1; i++) {
      zonali0[i] = adum[i]*pmarki[i];
    }
    _mm_free(adum);
    _mm_free(adum2);
#else
