...
#pragma omp parallel for num_threads(NUM_THREADS)
        for (int i=ipsi_in1; i<ipsi_out1+1; i++) {
            ...
            for (int j=ii+1; j<ii+jt+1; j++) {
              for (int k=1; k<mzeta+1; k++) {
		evector[3*(mzeta+1)*j+3*k+0] += phip00[i]/r; // phip00 = dphi/dr
              }
            }
...
#pragma omp parallel for
    ...
    for (int i=ipsi_in; i<ipsi_out+1; i++){
      for (int j=1; j<mtheta[i]+1; j++){
	for (int k=1; k<mzeta+1; k++){
	  if (fabs(evector[3*(igrid[i]+j-igrid_in)*(mzeta+1)+3*k+0])>max_e1)
	    max_e1 = fabs(evector[3*(igrid[i]+j-igrid_in)*(mzeta+1)+3*k+0]);
	  if (fabs(evector[3*(igrid[i]+j-igrid_in)*(mzeta+1)+3*k+1])>max_e2)
	    max_e2 = fabs(evector[3*(igrid[i]+j-igrid_in)*(mzeta+1)+3*k+1]);
	  if (fabs(evector[3*(igrid[i]+j-igrid_in)*(mzeta+1)+3*k+2])>max_e3)
	    max_e3 = fabs(evector[3*(igrid[i]+j-igrid_in)*(mzeta+1)+3*k+2]);
	}
      }
    }
    real max_e1_g=0.0;
    real max_e2_g=0.0;
    real max_e3_g=0.0;
    MPI_Reduce(&max_e1, &max_e1_g, 1, MPI_MYREAL, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce(&max_e2,&max_e2_g, 1, MPI_MYREAL, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce(&max_e3,&max_e3_g, 1, MPI_MYREAL, MPI_MAX, 0, MPI_COMM_WORLD);
    
    if (parallel_decomp->mype==0)
      printf("istep=%d max e1=%e e2=%e e3=%e\n", istep, max_e1_g, max_e2_g, max_e3_g);
    */
