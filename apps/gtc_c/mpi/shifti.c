...
#if TWO_WEIGHTS
	    sendright[nparam*i+12] = z6[pos];
	    sendright[nparam*i+13] = z06[pos];
#endif
        }
...
#if TWO_WEIGHTS
	    sendleft[nparam*i+12] = z6[pos];
	    sendleft[nparam*i+13] = z06[pos];
#endif
        }
...
#if TWO_WEIGHTS
	    z6[pos] = recvleft[nparam*i+12];
	    z06[pos] = recvleft[nparam*i+13];
#endif
        }
...
#if TWO_WEIGHTS
	    z6[pos]  = recvright[nparam*i+12];
	    z06[pos] = recvright[nparam*i+13];
#endif
        }
	end_t = MPI_Wtime();
	shift_t_comm1 += (end_t - start_t);
