...
#else
			r = rgrid + ddelr*2.0*vring[kr-1]*sqrt(rtemi[i]*0.5/b);
			t = tgrid + ddelt*2.0*vring[kr-1]*sqrt(rtemi[i]*0.5/b);
#endif
...
int poisson(int iflag, gtc_bench_data_t *gtc_input) {
    ...
    real *tmp1 = malloc((mpsi+1)*sizeof(real));
    real *tmp2 = malloc((mpsi+1)*sizeof(real));
    real *rdene = malloc((mpsi+1)*sizeof(real));
    real *rteme = malloc((mpsi+1)*sizeof(real));

    //memcpy((void *)rdene, (void *)rden, (mpsi+1)*sizeof(real));
    //memcpy((void *)rteme, (void *)rtemi, (mpsi+1)*sizeof(real));
#pragma omp parallel for
    ...
    for (int i=0; i<mpsi+1; i++){
      tmp1[i] = pow(qion,2)*rden[i]/rtemi[i];
      tmp2[i] = 1.0/(tmp1[i]*(1.0-gamma)+rdene[i]/rteme[i]);
    }
...
#pragma omp parallel for
	  ...
	  dentmp[i]  = qion*densityi[i*(mzeta+1)+k];
#else
	  dentmp[i]  = qion*densityi[i*(mzeta+1)+k]*rden[i];
#endif
...
#if UNIFORM_PARTICLE_LOADING
	    dentmp[igrid[i]+j] = qion*densityi[(igrid[i]+j)*(mzeta+1)+k];
#else
	    dentmp[igrid[i]+j] = qion*densityi[(igrid[i]+j)*(mzeta+1)+k]*rden[i];
#endif
            phitmp[igrid[i]+j] = dentmp[igrid[i]+j]*tmp2[i];
          }
	}
#endif
...
#pragma omp parallel for
	  for (int i=ipsi_nover_in_radiald; i<ipsi_nover_out_radiald+1; i++){
	    for (int j=1; j<mtheta[i]+1; j++){
	      real perr_i = (ptilde[igrid[i]+j-igrid_in] - gamma*phitmp[igrid[i]+j-igrid_in])*tmp1[i];
	      phitmp[igrid[i]+j-igrid_in] = (dentmp[igrid[i]+j-igrid_in]+perr_i)*tmp2[i];
	    }
	  }
#endif
            /* radial boundary */
...
#pragma omp parallel for
    ...
    if (parallel_decomp->mype==0) {
      char filename[50];
      sprintf(filename, "poisson_phi0.txt");
      FILE *fp;
      fp = fopen(filename, "w");
      for (int j=0; j<nloc_over; j++){
    ...
    if (parallel_decomp->mype==1){
      char filename[50];
      sprintf(filename, "poisson_phi1.txt");
      FILE *fp;
      fp = fopen(filename, "w");
      for (int j=0; j<nloc_over; j++){
