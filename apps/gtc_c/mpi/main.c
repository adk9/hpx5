...
int main (int argc, char** argv) {
    ...
    /* set up equilibirum data, e.g.,background temperature, density */
    equilibrium(gtc_input);
    ...
    /* output radial profile, e.g., temperature, density */
    radial_profile(gtc_input);
    ...
    for (istep=1; istep<=mstep; istep++) {
	    ...
	    /* apply phase space remapping to reduce noise */
            elt = timer();
    ...
    real poisson_fixghost_t_g = 0.0;
    real shift_t_comp_g=0.0;
    real shift_t_comm1_g=0.0;
    real shift_t_comm2_g=0.0;
    MPI_Reduce(&poisson_fixghost_t, &poisson_fixghost_t_g, 1, MPI_MYREAL, MPI_MAX, 0, MPI_COMM_WORLD);
    ...
    if (mype == 0) {
        ...
        fprintf(stderr, "shift_t_comp=%9.6f shift_t_comm1=%9.6f shift_t_comm2=%9.6f poisson_fixghost_t=%9.6f\n",
                shift_t_comp, shift_t_comm1, shift_t_comm2, poisson_fixghost_t);
        fprintf(stderr, "shift_t_comp_g=%9.6f shift_t_comm1_g=%9.6f shift_t_comm2_g=%9.6f poisson_fixghost_t_g=%9.6f\n",
                shift_t_comp_g, shift_t_comm1_g, shift_t_comm2_g, poisson_fixghost_t_g);
