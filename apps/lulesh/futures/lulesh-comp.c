...
void CalcVolumeForceForElems(int rank)
{
  ...
  if (numElem != 0) {
    ...
    double *sigxx = hpx_alloc(sizeof(double)*numElem);
    double *sigyy = hpx_alloc(sizeof(double)*numElem);
    double *sigzz = hpx_alloc(sizeof(double)*numElem);
    double *determ = hpx_alloc(sizeof(double)*numElem); 
...
void CalcHourglassControlForElems(Domain *domain, double determ[], double hgcoef)
{
  ...
  double *dvdx = hpx_alloc(sizeof(double)*numElem8);
  double *dvdy = hpx_alloc(sizeof(double)*numElem8);
  double *dvdz = hpx_alloc(sizeof(double)*numElem8);
  double *x8n = hpx_alloc(sizeof(double)*numElem8);
  double *y8n = hpx_alloc(sizeof(double)*numElem8);
  double *z8n = hpx_alloc(sizeof(double)*numElem8);
...
void LagrangeElements(int rank)
{
  ...
  domain->vnew = hpx_alloc(sizeof(double)*numElem);
...
void CalcLagrangeElements(Domain *domain)
{
  ...
  if (numElem > 0) {
    ...
    domain->dxx = hpx_alloc(sizeof(double)*numElem);
    domain->dyy = hpx_alloc(sizeof(double)*numElem);
    domain->dzz = hpx_alloc(sizeof(double)*numElem);
...
void CalcQForElems(int rank)
{
  ...
  if (numElem != 0) {
    ...
    domain->delv_xi = hpx_alloc(sizeof(double)*allElem);
    domain->delv_eta = hpx_alloc(sizeof(double)*allElem);
    domain->delv_zeta = hpx_alloc(sizeof(double)*allElem);

    domain->delx_xi = hpx_alloc(sizeof(double)*numElem);
    domain->delx_eta = hpx_alloc(sizeof(double)*numElem);
    domain->delx_zeta = hpx_alloc(sizeof(double)*numElem);
    ...
    hpx_free(domain->delx_zeta);
    hpx_free(domain->delx_eta);
    hpx_free(domain->delx_xi);

    hpx_free(domain->delv_zeta);
    hpx_free(domain->delv_eta);
    hpx_free(domain->delv_xi);
			...
			double *qq_old, double *ql_old, double rho0, double eosvmax, int length)
{
  ...
  double *pHalfStep = hpx_alloc(sizeof(double)*length);
...
void EvalEOSForElems(Domain *domain, double *vnewc, int numElem)
{
  ...
  double *e_old = hpx_alloc(sizeof(double)*numElem);
  double *delvc = hpx_alloc(sizeof(double)*numElem);
  double *p_old = hpx_alloc(sizeof(double)*numElem);
  double *q_old = hpx_alloc(sizeof(double)*numElem);
  double *compression = hpx_alloc(sizeof(double)*numElem);
  double *compHalfStep = hpx_alloc(sizeof(double)*numElem);
  double *qq_old = hpx_alloc(sizeof(double)*numElem);
  double *ql_old = hpx_alloc(sizeof(double)*numElem);
  double *work = hpx_alloc(sizeof(double)*numElem);
  double *p_new = hpx_alloc(sizeof(double)*numElem);
  double *e_new = hpx_alloc(sizeof(double)*numElem);
  double *q_new = hpx_alloc(sizeof(double)*numElem);
  double *bvc = hpx_alloc(sizeof(double)*numElem);
  double *pbvc = hpx_alloc(sizeof(double)*numElem);
  ...
  hpx_free(ql_old);
  hpx_free(qq_old);
  hpx_free(compHalfStep);
  hpx_free(compression);
  hpx_free(q_old);
  hpx_free(p_old);
  hpx_free(delvc);
  hpx_free(e_old);
...
void ApplyMaterialPropertiesForElems(Domain *domain)
{
  ...
  if (numElem != 0) {
    double eosvmin = domain->eosvmin;
    double eosvmax = domain->eosvmax;
    double *vnewc = hpx_alloc(sizeof(double)*numElem);
