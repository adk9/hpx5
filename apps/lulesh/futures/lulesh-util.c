	       ...
	       int nDomains, int maxcycles)
{
  ...
  // Elem-centered
  domain->matElemlist = hpx_alloc(sizeof(int)*domElems);
  domain->nodelist = hpx_alloc(sizeof(int)*8*domElems); 

  domain->lxim   = hpx_alloc(sizeof(int)*domElems);
  domain->lxip   = hpx_alloc(sizeof(int)*domElems); 
  domain->letam  = hpx_alloc(sizeof(int)*domElems); 
  domain->letap  = hpx_alloc(sizeof(int)*domElems); 
  domain->lzetam = hpx_alloc(sizeof(int)*domElems); 
  domain->lzetap = hpx_alloc(sizeof(int)*domElems);

  domain->elemBC = hpx_alloc(sizeof(int)*domElems);
  
  domain->e        = hpx_alloc(sizeof(double)*domElems);
  domain->p        = hpx_alloc(sizeof(double)*domElems);
  domain->q        = hpx_alloc(sizeof(double)*domElems);
  domain->ql       = hpx_alloc(sizeof(double)*domElems);
  domain->qq       = hpx_alloc(sizeof(double)*domElems);
  domain->v        = hpx_alloc(sizeof(double)*domElems);
  domain->volo     = hpx_alloc(sizeof(double)*domElems);
  domain->delv     = hpx_alloc(sizeof(double)*domElems);
  domain->vdov     = hpx_alloc(sizeof(double)*domElems);
  domain->arealg   = hpx_alloc(sizeof(double)*domElems); 
  domain->ss       = hpx_alloc(sizeof(double)*domElems); 
  domain->elemMass = hpx_alloc(sizeof(double)*domElems); 

  // Node-centered
  domain->x         = hpx_alloc(sizeof(double)*domNodes);
  domain->y         = hpx_alloc(sizeof(double)*domNodes);
  domain->z         = hpx_alloc(sizeof(double)*domNodes);
  domain->xd        = hpx_alloc(sizeof(double)*domNodes);
  domain->yd        = hpx_alloc(sizeof(double)*domNodes);
  domain->zd        = hpx_alloc(sizeof(double)*domNodes);
  domain->xdd       = hpx_alloc(sizeof(double)*domNodes); 
  domain->ydd       = hpx_alloc(sizeof(double)*domNodes); 
  domain->zdd       = hpx_alloc(sizeof(double)*domNodes);
  domain->fx        = hpx_alloc(sizeof(double)*domNodes); 
  domain->fy        = hpx_alloc(sizeof(double)*domNodes);
  domain->fz        = hpx_alloc(sizeof(double)*domNodes); 
  domain->nodalMass = hpx_alloc(sizeof(double)*domNodes); 
  ...
  // boundary nodesets
  if (colLoc == 0) 
    domain->symmX = hpx_alloc(sizeof(int)*edgeNodes*edgeNodes);
  if (rowLoc == 0) 
    domain->symmY = hpx_alloc(sizeof(int)*edgeNodes*edgeNodes);
  if (planeLoc == 0) 
    domain->symmZ = hpx_alloc(sizeof(int)*edgeNodes*edgeNodes);
  ...
  if (planeMin) {
    // #5
    domain->dataRecvTF[4] = hpx_alloc(PLANEBUFFERSZ); 
    domain->dataRecvTT[4] = hpx_alloc(PLANEBUFFERSZ);
    recvTF[nrTF++] = 4;
    recvTT[nrTT++] = 4;

    domain->dataSendTF[4] = hpx_alloc(FUTURESZ);
    domain->dataSendTT[4] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[4] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 4;
    sendTT[nsTT++] = 4;
    sendFF[nsFF++] = 4;
  ...
  if (planeMax) {
    // #22
    domain->dataRecvTF[21] = hpx_alloc(PLANEBUFFERSZ);
    domain->dataRecvTT[21] = hpx_alloc(PLANEBUFFERSZ);
    domain->dataRecvFF[21] = hpx_alloc(PLANEBUFFERSZ);
    recvTF[nrTF++] = 21;
    recvTT[nrTT++] = 21;
    recvFF[nrFF++] = 21;

    domain->dataSendTF[21] = hpx_alloc(FUTURESZ);
    domain->dataSendTT[21] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 21;
    sendTT[nsTT++] = 21;
  ...
  if (rowMin) {
    // #11 
    domain->dataRecvTF[10] = hpx_alloc(PLANEBUFFERSZ);
    domain->dataRecvTT[10] = hpx_alloc(PLANEBUFFERSZ);
    recvTF[nrTF++] = 10;
    recvTT[nrTT++] = 10; 

    domain->dataSendTF[10] = hpx_alloc(FUTURESZ);
    domain->dataSendTT[10] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[10] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 10;
    sendTT[nsTT++] = 10;
    sendFF[nsFF++] = 10;
  ...
  if (rowMax) {
    // #16
    domain->dataRecvTF[15] = hpx_alloc(PLANEBUFFERSZ);
    domain->dataRecvTT[15] = hpx_alloc(PLANEBUFFERSZ);
    domain->dataRecvFF[15] = hpx_alloc(PLANEBUFFERSZ);
    recvTF[nrTF++] = 15;
    recvTT[nrTT++] = 15;
    recvFF[nrFF++] = 15;

    domain->dataSendTF[15] = hpx_alloc(FUTURESZ);
    domain->dataSendTT[15] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 15;
    sendTT[nsTT++] = 15;
  ...
  if (colMin) {
    // #13 
    domain->dataRecvTF[12] = hpx_alloc(PLANEBUFFERSZ);
    domain->dataRecvTT[12] = hpx_alloc(PLANEBUFFERSZ);
    recvTF[nrTF++] = 12;
    recvTT[nrTT++] = 12;

    domain->dataSendTF[12] = hpx_alloc(FUTURESZ);
    domain->dataSendTT[12] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[12] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 12;
    sendTT[nsTT++] = 12;
    sendFF[nsFF++] = 12;
  ...
  if (colMax) {
    // #14
    domain->dataRecvTF[13] = hpx_alloc(PLANEBUFFERSZ);
    domain->dataRecvTT[13] = hpx_alloc(PLANEBUFFERSZ);
    domain->dataRecvFF[13] = hpx_alloc(PLANEBUFFERSZ);
    recvTF[nrTF++] = 13;
    recvTT[nrTT++] = 13;
    recvFF[nrFF++] = 13;

    domain->dataSendTF[13] = hpx_alloc(FUTURESZ);
    domain->dataSendTT[13] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 13;
    sendTT[nsTT++] = 13;
  ...
  if (rowMin & colMin) {
    // #10 
    domain->dataRecvTF[9] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 9;

    domain->dataSendTF[9] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[9] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 9;
    sendFF[nsFF++] = 9;
  ...
  if (rowMin & planeMin) {
    // #2
    domain->dataRecvTF[1] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 1; 

    domain->dataSendTF[1] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[1] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 1;
    sendFF[nsFF++] = 1;
  ...
  if (colMin & planeMin) {
    // #4 
    domain->dataRecvTF[3] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 3;

    domain->dataSendTF[3] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[3] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 3;
    sendFF[nsFF++] = 3;
  ...
  if (rowMax & colMax) {
    // #17
    domain->dataRecvTF[16] = hpx_alloc(EDGEBUFFERSZ);
    domain->dataRecvFF[16] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 16;
    recvFF[nrFF++] = 16;

    domain->dataSendTF[16] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 16;
  ...
  if (rowMax & planeMax) {
    // #25
    domain->dataRecvTF[24] = hpx_alloc(EDGEBUFFERSZ);
    domain->dataRecvFF[24] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 24;
    recvFF[nrFF++] = 24;

    domain->dataSendTF[24] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 24;
  ...
  if (colMax & planeMax) {
    // #23
    domain->dataRecvTF[22] = hpx_alloc(EDGEBUFFERSZ);
    domain->dataRecvFF[22] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 22;
    recvFF[nrFF++] = 22;

    domain->dataSendTF[22] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 22;
  ...
  if (rowMax & colMin) {
    // #15
    domain->dataRecvTF[14] = hpx_alloc(EDGEBUFFERSZ);
    domain->dataRecvFF[14] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 14;
    recvFF[nrFF++] = 14;

    domain->dataSendTF[14] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 14;
  ...
  if (rowMin & planeMax) {
    // #19
    domain->dataRecvTF[18] = hpx_alloc(EDGEBUFFERSZ);
    domain->dataRecvFF[18] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 18;
    recvFF[nrFF++] = 18;

    domain->dataSendTF[18] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 18;
  ...
  if (colMin & planeMax) {
    // #21
    domain->dataRecvTF[20] = hpx_alloc(EDGEBUFFERSZ);
    domain->dataRecvFF[20] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 20;
    recvFF[nrFF++] = 20;
    
    domain->dataSendTF[20] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 20;
  ...
  if (rowMin & colMax) {
    // #12
    domain->dataRecvTF[11] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 11;

    domain->dataSendTF[11] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[11] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 11;
    sendFF[nsFF++] = 11;
  ...
  if (rowMax & planeMin) {
    // #8
    domain->dataRecvTF[7] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 7;
    
    domain->dataSendTF[7] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[7] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 7;
    sendFF[nsFF++] = 7;
  ...
  if (colMax & planeMin) {
    // #6
    domain->dataRecvTF[5] = hpx_alloc(EDGEBUFFERSZ);
    recvTF[nrTF++] = 5;
    
    domain->dataSendTF[5] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[5] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 5;
    sendFF[nsFF++] = 5;
  ...
  if (rowMin & colMin & planeMin) {
    // #1
    domain->dataRecvTF[0] = hpx_alloc(CORNERBUFFERSZ); 
    recvTF[nrTF++] = 0; 

    domain->dataSendTF[0] = hpx_alloc(FUTURESZ); 
    domain->dataSendFF[0] = hpx_alloc(FUTURESZ);    
    sendTF[nsTF++] = 0; 
    sendFF[nsFF++] = 0;
  ...
  if (rowMin & colMin & planeMax) {
    // #18
    domain->dataRecvTF[17] = hpx_alloc(CORNERBUFFERSZ);
    domain->dataRecvFF[17] = hpx_alloc(CORNERBUFFERSZ);
    recvTF[nrTF++] = 17;
    recvFF[nrFF++] = 17;

    domain->dataSendTF[17] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 17;
  ...
  if (rowMin & colMax & planeMin) {
    // #3
    domain->dataRecvTF[2] = hpx_alloc(CORNERBUFFERSZ);
    recvTF[nrTF++] = 2;

    domain->dataSendTF[2] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[2] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 2;
    sendFF[nsFF++] = 2;
  ...
  if (rowMin & colMax & planeMax) {
    // #20 
    domain->dataRecvTF[19] = hpx_alloc(CORNERBUFFERSZ);
    domain->dataRecvFF[19] = hpx_alloc(CORNERBUFFERSZ);
    recvTF[nrTF++] = 19;
    recvFF[nrFF++] = 19;

    domain->dataSendTF[19] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 19;
  ...
  if (rowMax & colMin & planeMin) {
    // #7 
    domain->dataRecvTF[6] = hpx_alloc(CORNERBUFFERSZ);    
    recvTF[nrTF++] = 6;

    domain->dataSendTF[6] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[6] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 6;
    sendFF[nsFF++] = 6;
  ...
  if (rowMax & colMin & planeMax) {
    // #24
    domain->dataRecvTF[23] = hpx_alloc(CORNERBUFFERSZ);
    domain->dataRecvFF[23] = hpx_alloc(CORNERBUFFERSZ);
    recvTF[nrTF++] = 23;
    recvFF[nrFF++] = 23;

    domain->dataSendTF[23] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 23;
  ...
  if (rowMax & colMax & planeMin) {
    // #9 
    domain->dataRecvTF[8] = hpx_alloc(CORNERBUFFERSZ);
    recvTF[nrTF++] = 8;

    domain->dataSendTF[8] = hpx_alloc(FUTURESZ);
    domain->dataSendFF[8] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 8;
    sendFF[nsFF++] = 8;
  ...
  if (rowMax & colMax & planeMax) {
    // #26
    domain->dataRecvTF[25] = hpx_alloc(CORNERBUFFERSZ);
    domain->dataRecvFF[25] = hpx_alloc(CORNERBUFFERSZ);
    recvTF[nrTF++] = 25;
    recvFF[nrFF++] = 25;

    domain->dataSendTF[25] = hpx_alloc(FUTURESZ);
    sendTF[nsTF++] = 25;
...
{
  hpx_free(domain->matElemlist);
  hpx_free(domain->nodelist);
  hpx_free(domain->lxim);
  hpx_free(domain->lxip);
  hpx_free(domain->letam);
  hpx_free(domain->letap);
  hpx_free(domain->lzetam);
  hpx_free(domain->lzetap);
  hpx_free(domain->elemBC);
  hpx_free(domain->e);
  hpx_free(domain->p);
  hpx_free(domain->q);
  hpx_free(domain->ql);
  hpx_free(domain->qq);
  hpx_free(domain->v);
  hpx_free(domain->volo);
  hpx_free(domain->delv);
  hpx_free(domain->arealg);
  hpx_free(domain->ss);
  hpx_free(domain->elemMass);
  hpx_free(domain->x);
  hpx_free(domain->y);
  hpx_free(domain->z);
  hpx_free(domain->xd);
  ...
  hpx_free(domain->fy);
  hpx_free(domain->fz);
  hpx_free(domain->nodalMass);

  if (domain->colLoc)
    hpx_free(domain->symmX);

  if (domain->rowLoc)
    hpx_free(domain->symmY);

  if (domain->planeLoc)
    hpx_free(domain->symmZ);

  int i; 
  for (i = 0; i < 26; i++) {
    hpx_free(domain->dataRecvTF[i]);
    hpx_free(domain->dataRecvTT[i]);
    hpx_free(domain->dataRecvFF[i]);

    hpx_free(domain->dataSendTF[i]);
    hpx_free(domain->dataSendTT[i]);
    hpx_free(domain->dataSendFF[i]);
  }
}
