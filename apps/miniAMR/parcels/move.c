 ...
 *    * (later) diamond, solid diamond */
   ...
   for (in = 0; in < ld->sorted_index[ld->num_refine+1]; in++) {
      ...
      if ((bp = &ld->blocks[n])->number >= 0) {
         sz = ld->p2[ld->num_refine - bp->level]; /* half size of block */
         cor[0][0] = ((double) (bp->cen[0] - sz))/((double) ld->mesh_size[0]);
         cor[0][1] = ((double) (bp->cen[0] + sz))/((double) ld->mesh_size[0]);
         cor[1][0] = ((double) (bp->cen[1] - sz))/((double) ld->mesh_size[1]);
         cor[1][1] = ((double) (bp->cen[1] + sz))/((double) ld->mesh_size[1]);
         cor[2][0] = ((double) (bp->cen[2] - sz))/((double) ld->mesh_size[2]);
         cor[2][1] = ((double) (bp->cen[2] + sz))/((double) ld->mesh_size[2]);
         for (o = 0; o < ld->num_objects; o++) {
            op = &ld->objects[o];
            if (bp->refine > 0 ||
         ...
         if (bp->level == ld->num_refine && bp->refine == 1) {
            bp->refine = 0;
