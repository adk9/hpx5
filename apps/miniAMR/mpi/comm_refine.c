   ...
   for (dir = 0; dir < 3; dir++) {
      ...
      for (i = 0; i < num_comm_partners[dir]; i++) {
         for (n = 0; n < comm_num[dir][i]; n++)
            send_int[n] = blocks[comm_block[dir][comm_index[dir][i]+n]].refine;
...
{
   int i, n, c, offset, dir, which, face;
   int *send_int = (int *) send_buff;
   int *recv_int = (int *) recv_buff;
   block *bp;
   MPI_Status status;
   ...
   for (dir = 0; dir < 3; dir++) {
      for (i = 0; i < num_comm_partners[dir]; i++)
         MPI_Irecv(&recv_int[comm_index[dir][i]], comm_num[dir][i],
                   MPI_INTEGER, comm_partner[dir][i], 2, MPI_COMM_WORLD,
                   &request[i]);
      ...
      for (i = 0; i < num_comm_partners[dir]; i++) {
         ...
         MPI_Send(send_int, comm_num[dir][i], MPI_INTEGER,
                  comm_partner[dir][i], 2, MPI_COMM_WORLD);
      }
