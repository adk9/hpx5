         ...
         if (blocks[n].number >= 0 && blocks[n].level == level) {
            ...
            if (bp->refine == 1) {
               ...
               if (pp->parent_node == my_pe) {
                  ...
                  del_par_list(&par_b, (-2-bp->parent), n, bp->child_number,
                               bp->parent_node);
                  add_par_list(&par_b, (-2-bp->parent), (-1-p),
               ...
               for (o = 0; o < 8; o++) {
                  ...
                  if (level < (num_refine-1))
                     offset = p2[num_refine - level - 2];
                  i1 *= x_block_half;
                  j1 *= y_block_half;
                  k1 *= z_block_half;
      ...
      for (p = 0; p < max_active_parent; p++)
         if ((pp = &parents[p])->number >= 0 && pp->level == level &&
            ...
            // else communicate this change later
               ...
               del_par_list(&par_b, (-2-bp->parent), (-1-p), bp->child_number,
                            bp->parent_node);
               add_par_list(&par_b, (-2-bp->parent), n, bp->child_number,
                            bp->parent_node, 0);
            }
            ...
            if (bp->nei[c][0][0] < 0 && bp->nei_refine[c] == -1) {
               ...
               if (bp->nei_level[c] == level) {
                  ...
                  printf("%d ERROR: con nei block %d pe %d bad b %d %d l %d %d\n",
                         my_pe, c, pe, n, bp->number, level, bp->nei_level[c]);
                  exit(-1);
               }
               bp->nei_refine[c] = 0;
            }
