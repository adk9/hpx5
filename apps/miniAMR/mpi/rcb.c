...
{
   int npx1, npy1, npz1, np1, nfac, fac[25], fact;
   int i, j, k, m, n, dir, in;
...
{
   int f, s, sp, fp, i, j[25], k, l, rb, lev, nfac, block_size, type, type1,
       par[25], start[25];
   double t1, t2, t3, t4;
   MPI_Status status;
            ...
            if (bp->parent_node == my_pe) {
               for (p = 0; p < max_active_parent; p++)
                  if (parents[p].number == -2 - bp->parent) {
                     bp->parent = p;
                     parents[p].child[bp->child_number] = n;
