...
static __attribute__((constructor)) void _dimacs_register_actions() {
  HPX_REGISTER_ACTION(dimacs_checksum_action, &dimacs_checksum);
  HPX_REGISTER_ACTION(_dimacs_visit_vertex_action, &_dimacs_visit_vertex);
  HPX_REGISTER_ACTION(_dimacs_send_dist_action, &_dimacs_send_dist);
}
