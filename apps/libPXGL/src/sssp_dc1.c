...
static bool _maybe_process_one(sssp_queue_t *const q) {
  hpx_addr_t v;
  distance_t d;
...
static int _handle_queue_action(sssp_queue_t *const *const *const args) {
  ...
  for(size_t i = 0; i < empty_factors[thread_id] && i < current_size && _maybe_process_one(queue); ++i) ;
