...
int call_kla_sssp_action(const call_sssp_args_t *const args) {
  ...
  while(true) {
    ...
    if (_get_termination() == COUNT_TERMINATION) {
      ...
      //printf("Starting initialization termination detection  bcast.\n");
      hpx_bcast(_initialize_termination_detection, init_termination_count_lco, NULL, 0);
      //printf("Waiting on bcast.\n");
      hpx_lco_wait(init_termination_count_lco);
      hpx_lco_delete(init_termination_count_lco, HPX_NULL);
      //printf("Done initialization termination detection  bcast.\n");
      const hpx_addr_t active_counts_termination_lco = hpx_lco_future_new(0);
      hpx_bcast(_delta_sssp_increase_active_counts, active_counts_termination_lco, NULL, 0);
      ...
      hpx_bcast(_visit_all_in_current_klevel, HPX_NULL, &kla_visit_args, sizeof(kla_visit_args));
...
static HPX_CONSTRUCTOR void _sssp_kla_register_actions() {
  HPX_REGISTER_ACTION(call_kla_sssp_action , &call_kla_sssp);
  HPX_REGISTER_ACTION(_visit_all_in_klevel_buffer_action, 
                      & _visit_all_in_klevel_buffer);
  HPX_REGISTER_ACTION(_visit_all_in_current_klevel_action, 
		      & _visit_all_in_current_klevel);
  HPX_REGISTER_ACTION(_sssp_klevel_process_vertex_action, 
                      &_sssp_klevel_process_vertex);
  HPX_REGISTER_ACTION(_sssp_visit_vertex_klevel_action,
		      &_sssp_visit_vertex_klevel);
}
