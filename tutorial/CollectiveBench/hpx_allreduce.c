...
static int _reduce_action(const InitArgs *args, size_t n) {
  ...
  for (size = 1; size <= max_msg_size; size *= 2) {
    ...
    for (int i = 0; i < iterations + skip ; i++) {
      ...
      hpx_lco_set(ld->collVal, sizeof(double), &dsize, HPX_NULL, HPX_NULL);
...
int main(int argc, char *argv[])
{
  ...
  HPX_REGISTER_ACTION(HPX_DEFAULT, HPX_MARSHALLED, _reduce, _reduce_action, HPX_POINTER, HPX_SIZE_T);
