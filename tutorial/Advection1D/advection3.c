...
int point_integrate_handler(point_t *P, hpx_addr_t grid, int i); 
...
HPX_ACTION(HPX_DEFAULT, HPX_PINNED, point_integrate, 
           point_integrate_handler, HPX_POINTER, HPX_ADDR, HPX_INT); 
...
int point_integrate_handler(point_t *P, hpx_addr_t grid, int i) {
  ...
  // wait for my local reduction to complete 
  double temp; 
  hpx_lco_get(P->reduce, sizeof(double), &temp); 
  P->T += temp; 

  // reset reduction LCO for the next timestep
...
void grid_integrate(hpx_addr_t grid) {
  ...
  for (int i = 0; i < N; i++) {
    hpx_addr_t point = grid_element_at(grid, i); 
    hpx_call(point, point_integrate, done, &grid, &i); 
  }
