...
static int _print_gas(int i, hpx_addr_t addr, void *args){
  ELEMENT e = *((ELEMENT*)addr);
  printf("%d %d\n", i, e);
  return HPX_SUCCESS;
}
...
static int nested_for_handler(void) {
  ...
  e = hpx_nested_for_sync(_print_gas, 0, blk_num * elem_per_blk, blk_size, offset, sizeof(ELEMENT), 0,
                          NULL, array);
  if (HPX_SUCCESS != e)
    return e;
