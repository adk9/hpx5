...
static int _allreduce_action(hpx_addr_t *args) {
  ...
  double buf = rand();
  hpx_addr_t root = hpx_lco_chan_array_at(channels, 0, 1);
  hpx_lco_chan_send(root, sizeof(buf), &buf, HPX_NULL, HPX_NULL);

  hpx_addr_t chan = hpx_lco_chan_array_at(channels, HPX_LOCALITY_ID, 1);
  double *result;
...
static int _main_action(void *args) {
  ...
  for (int k=0; k<ranks; ++k) {
    ...
    rank = hpx_lco_chan_array_at(channels, k, 1);
  ...
  for (int k=0; k<ranks; ++k) {
    if (k == HPX_LOCALITY_ID) continue;
    rank = hpx_lco_chan_array_at(channels, HPX_LOCALITY_ID, 1);
