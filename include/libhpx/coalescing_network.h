...
static void _send_all(coalesced_network_t *coalesced_network) {
  ...
  if(coalesced_network_parcel_queue_size(coalesced_network) > 0) {
   ...
   for (rank = 0; rank < HPX_LOCALITIES; rank++) {
     ...
     hpx_parcel_t *p = parcel_new(target, _demultiplexer, 0, 0, pid, coalesced_network->coalesced_buffer[rank] , coalesced_network->total_byte_count[rank]); 
     _prepare(p);
...
static int coalesced_network_send(void *network,  hpx_parcel_t *p) {
  ...
  //if (current_coalescing_queue_size == COALESCING_SIZE ) {
  ...
  sync_store(&coalesced_network->previous_queue_size, coalesced_network_parcel_queue_size(coalesced_network), SYNC_RELAXED);
