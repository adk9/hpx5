...
if [ "$OP" == "run" ]; then
	...
	$RUNCMD ./sssp -q $TIMEOUT -c -z 40000 $HPXCORES --hpx-heap=$((1024 * 1024 * 1024 * 3)) --hpx-sendlimit=128 --hpx-transport=$HPXMODE_AXIS --hpx-recvlimit=512 "$INPUT_DIR/Random4-n.22.0.gr" "$INPUT_DIR/Random4-n.22.0.ss" || { echo 'SSSP test failed' ; exit 1; }
        ...
        # Distributed control
        $RUNCMD ./sssp -q $TIMEOUT -d $HPXCORES --hpx-heap=$((1024 * 1024 * 1024 * 3)) --hpx-sendlimit=128 --hpx-transport=$HPXMODE_AXIS --hpx-recvlimit=512 $INPUT_DIR/Random4-n.12.0.gr $INPUT_DIR/Random4-n.12.0.ss || { echo 'SSSP test failed' ; exit 1; }
fi
