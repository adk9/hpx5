...
static hpx_parcel_t *
_try_bind(worker_t *w, hpx_parcel_t *p) {
  dbg_assert(p);
...
hpx_thread_get_tls_id(void) {
  worker_t *w = self;
  ustack_t *stack = parcel_get_stack(w->current);
  if (stack->tls_id < 0) {
