...
bool apex_set_new_thread_caps_hysteresis(int count, apex_profile values[count]) {
  ...
  double moving_max =
      ((moving_average[max_rank] * (window_size-1)) + last_period) / window_size;
  // set each node thread cap to a relative multiple of that
  for (int i = 0; i < count; ++i) { 
    int old_cap = thread_caps[i];
    // how much accumulated in the last period?
    ...
    moving_average[i] =
        ((moving_average[i] * (window_size-1)) + last_period) / window_size;
...
int apex_allreduce_action(void *unused) {
  ...
  if (ready && good_values) {
    ...
    for (int i = 0; i < num_ranks; ++i) {
      ...
      hpx_call(HPX_THERE(i), apex_set_local_cap, futures[i], &(thread_caps[i]),
               sizeof(thread_caps[i]));
    }
...
int apex_periodic_output(apex_context const context) {
  ...
  printf("Iterations: calls=%lu mean=%f +/- %f, min=%f (%d), max=%f (%d)\r",
         (unsigned long)reduced_value.calls, avg, stddev, reduced_value.minimum,
         min_rank, reduced_value.maximum, max_rank);
  fflush(stdout);
...
void apex_global_setup(apex_profiler_type type, void* in_action) {
  ...
  fprintf(graph_output,"\"calls\"\t\"mean\"\t\"stddev\"\t\"min\"\t\"min_rank"
          "\"\t\"max\"\t\"max_rank\"\n");
  fflush(graph_output);
