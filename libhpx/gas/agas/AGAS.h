...
class AGAS : public GAS {
 ...
 public:
  ...
  static AGAS* Instance() {
    dbg_assert(here && here->gas && here->gas->type() == HPX_GAS_AGAS);
    return static_cast<AGAS*>(here->gas);
  }
 ...
 public:
  ...
  static int FreeAsyncHandler(hpx_addr_t base) {
    Instance()->freeAsync(base);
    return HPX_SUCCESS;
  }
 ...
 private:
  ...
  /// cleans up all of the blocks associated with the allocation in a
  /// hierarchical way.
  ///
  /// for each segment
  ///   for each block
