...
class BlockTranslationTable
{
 public:
  ...
  /// Get the number of blocks associated with the global virtual address.
  size_t getBlocks(GVA gva) const;

  /// Get the "segment" data associated with a mapping.
  ///
  /// The segment is the local virtual address of the mapping and the number of
  /// blocks.
  void* getSegment(GVA gva, size_t& blocks) const;
