...
{
  Entry entry;
  return (map_.find(gva, entry)) ? entry.owner : gva.home;
}
...
// bool
// BTT::getAttr(GVA gva, uint32_t *attr) const
// {
//   dbg_assert(attr);
//   Entry entry;
...
size_t
BTT::getBlocks(GVA gva) const
{
  Entry entry;
  return (map_.find(gva, entry)) ? entry.blocks : 0;
}
...
BTT::tryRemove(GlobalVirtualAddress gva, void*& lva)
{
  ...
  return (map_.update_fn(gva, fn)) ? (count == 0) : false;
}
