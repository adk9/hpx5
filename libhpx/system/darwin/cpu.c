...
int system_get_affinity_group_size(pthread_t thread, int *ncores) {
  ...
  // Ref: https://developer.apple.com/library/mac/releasenotes/Performance/RN-AffinityAPI/
  ...
  int kr = thread_policy_get(pthread_mach_thread_np(thread),
			     THREAD_AFFINITY_POLICY, (thread_policy_t)&policy, &count, &b);
  ...
  if (policy.affinity_tag != THREAD_AFFINITY_TAG_NULL) { // associated with a L2 cache
