...
static void _add_switch(uint64_t dpid, uint32_t tid, uint32_t nbuf,
  ...
  switch_t *s = NULL;
  LL_SEARCH(trema->switches, s, dpid, _cmd_dpid);
  if (!s) {
    s = malloc(sizeof(*s));
    LL_PREPEND(trema->switches, s);
    dbg_log("Adding new switch %# " PRIx64 ".\n", dpid);
  }
...
static void _start(void *args) {
  ...
  set_switch_ready_handler(_query_switch, NULL);
  set_features_reply_handler(_add_switch, trema);
  set_switch_disconnected_handler(_disable_switch, trema);
...
static void _delete(routing_t *routing) {
  ...
  // delete all switches
  LL_FOREACH_SAFE(trema->switches, s, tmp) {
    LL_DELETE(trema->switches, s);
    free(s);
  }
  ...
  int e = pthread_join(trema->ofthread, NULL);
  if (e)
...
static int _my_port(const routing *r) {
  photonAddr addr;
...
routing_t *routing_new_trema(void) {
  ...
  if (e)
    return dbg_error("could not start the OF controller thread.\n");
