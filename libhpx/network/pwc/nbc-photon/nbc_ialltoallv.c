...
int NBC_Ialltoallv(void* sendbuf, int *sendcounts, int *sdispls,
    ...
    MPI_Datatype recvtype, NBC_Comminfo* comm, NBC_Handle* handle) {
  ...
  if((sendcounts[rank] != 0) && !inplace) {
    ...
    res = NBC_Copy(sbuf, sendcounts[rank], sendtype, rbuf, recvcounts[rank], recvtype);
  ...
  for (i = 0; i < p; i++) {
    ...
    if(recvcounts[i] != 0) {
      ...
      res = NBC_Sched_recv(rbuf, false, recvcounts[i], recvtype, i, schedule, comm);
