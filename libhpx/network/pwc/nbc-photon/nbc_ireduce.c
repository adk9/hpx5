...
static __inline__ int red_sched_binomial(int rank, int p, int root, void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, 
		MPI_Op op, void *redbuf, NBC_Schedule *schedule, NBC_Handle *handle, NBC_Comminfo *comm);
static __inline__ int red_sched_chain(int rank, int p, int root, void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, 
		MPI_Op op, int ext, int size, NBC_Schedule *schedule, NBC_Handle *handle, int fragsize, NBC_Comminfo *comm);
  ...
  if(found == NULL) {
    ...
    switch(alg) {
      case NBC_RED_BINOMIAL:
        res = red_sched_binomial(rank, p, root, sendbuf, recvbuf, count, datatype, op, redbuf, schedule, handle, comm);
        break;
      case NBC_RED_CHAIN:
        res = red_sched_chain(rank, p, root, sendbuf, recvbuf, count, datatype, op, ext, size, schedule, handle, segsize, comm);
        break;
    }
...
static __inline__ int red_sched_binomial(int rank, int p, int root, void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, void *redbuf, NBC_Schedule *schedule, NBC_Handle *handle, NBC_Comminfo* comm) {
  ...
  for(r=1; r<=maxr; r++) {
    if((vrank % (1<<r)) == 0) {
      ...
      if(firstred) {
        ...
        res = NBC_Sched_send((char *)redbuf-(unsigned long)handle->tmpbuf, true, count, datatype, peer, schedule, comm);
      }
...
/* chain send ... */ 
static __inline__ int red_sched_chain(int rank, int p, int root, void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, int ext, int size, NBC_Schedule *schedule, NBC_Handle *handle, int fragsize, NBC_Comminfo* comm) {
  ...
  for(fragnum = 0; fragnum < numfrag; fragnum++) {
    ...
    if(vrank != p-1) {
      res = NBC_Sched_recv((char*)offset, true, thiscount, datatype, rpeer, schedule, comm);
    ...
    if(vrank != 0) {
      ...
      if(vrank == p-1) {
        res = NBC_Sched_send((char*)sendbuf+offset, false, thiscount, datatype, speer, schedule, comm);
      } else {
        res = NBC_Sched_send((char*)offset, true, thiscount, datatype, speer, schedule, comm);
      }
