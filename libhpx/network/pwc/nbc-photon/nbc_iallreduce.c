...
static __inline__ int allred_sched_diss(int rank, int p, int count, MPI_Datatype datatype, 
		void *sendbuf, void *recvbuf, MPI_Op op, NBC_Schedule *schedule, NBC_Handle *handle, NBC_Comminfo* comm);
static __inline__ int allred_sched_chain(int rank, int p, int count, MPI_Datatype datatype,
	       	void *sendbuf, void *recvbuf, MPI_Op op, int size, int ext, NBC_Schedule *schedule, 
		NBC_Handle *handle, int fragsize, NBC_Comminfo* comm);
static __inline__ int allred_sched_ring(int rank, int p, int count, MPI_Datatype datatype, 
		void *sendbuf, void *recvbuf, MPI_Op op, int size, int ext, NBC_Schedule *schedule, 
  ...
  if(found == NULL) {
    ...
    switch(alg) {
      case NBC_ARED_BINOMIAL:
        res = allred_sched_diss(rank, p, count, datatype, sendbuf, recvbuf, op, schedule, handle, comm);
        break;
      case NBC_ARED_RING:
        res = allred_sched_ring(rank, p, count, datatype, sendbuf, recvbuf, op, size, ext, schedule, handle, comm);
        break;
    }
...
static __inline__ int allred_sched_diss(int rank, int p, int count, MPI_Datatype datatype, 
  ...
  for(r=1; r<=maxr; r++) {
    if((vrank % (1<<r)) == 0) {
      ...
      if(peer<p) {
        res = NBC_Sched_recv(0, true, count, datatype, peer, schedule, comm);
      ...
      if(firstred) {
        ...
        res = NBC_Sched_send(sendbuf, false, count, datatype, peer, schedule, comm);
      } else {
        /* and the recvbuf in all remeining rounds */
        res = NBC_Sched_send(recvbuf, false, count, datatype, peer, schedule, comm);
      }
  ...
  if(vrank != 0) {
    for(r=0; r<maxr; r++) {
      if((vrank >= (1<<r)) && (vrank < (1<<(r+1)))) {
        ...
        res = NBC_Sched_recv(recvbuf, false, count, datatype, peer, schedule, comm);
        if(res != NBC_OK) { free(handle->tmpbuf); printf("Error in NBC_Sched_recv() (%i)\n", res); return res; }
      }
    }
    res = NBC_Sched_barrier(schedule);
...
static __inline__ int allred_sched_chain(int rank, int p, int count, MPI_Datatype datatype, 
  ...
  for(fragnum = 0; fragnum < bend; fragnum++) {
    ...
    if(fragnum < numfrag) {
      ...
      if(rank != p-1) {
        res = NBC_Sched_recv((char*)roffset, true, thiscount, datatype, rrpeer, schedule, comm);
      ...
      if(rank != 0) {
        ...
        if(rank == p-1) {
          res = NBC_Sched_send((char*)sendbuf+roffset, false, thiscount, datatype, srpeer, schedule, comm);
        } else {
          res = NBC_Sched_send((char*)roffset, true, thiscount, datatype, srpeer, schedule, comm);
        }
    ...
    if(fragnum >= bstart) {
      ...
      if(rank != 0) {
        res = NBC_Sched_recv((char*)recvbuf+boffset, false, thiscount, datatype, rbpeer, schedule, comm);
        if (NBC_OK != res) { printf("Error in NBC_Sched_recv() (%i)\n", res); return res; }
        res = NBC_Sched_barrier(schedule);
      }
      ...
      if(rank != p-1) {
        res = NBC_Sched_send((char*)recvbuf+boffset, false, thiscount, datatype, sbpeer, schedule, comm);
        if (NBC_OK != res) { printf("Error in NBC_Sched_send() (%i)\n", res); return res; }
        res = NBC_Sched_barrier(schedule);
      }
    }
...
static __inline__ int allred_sched_ring(int r, int p, int count, MPI_Datatype datatype,
	       	void *sendbuf, void *recvbuf, MPI_Op op, int size, int ext, NBC_Schedule *schedule,
	       	NBC_Handle *handle, NBC_Comminfo* comm) {
  int i; /* runner */
   ...
   */
  {
    ...
    do {
      ...
      if(round == 0) {
        NBC_Sched_send((char*)sendbuf+soffset, false, segsizes[selement], datatype, speer, schedule, comm);
        //printf("[%i] round %i - sending %i\n", r, round, selement);
      } else {
        NBC_Sched_send((char*)recvbuf+soffset, false, segsizes[selement], datatype, speer, schedule, comm);
        //printf("[%i] round %i - sending %i\n", r, round, selement);
      }
      NBC_Sched_recv((char*)recvbuf+roffset, false, segsizes[relement], datatype, rpeer, schedule, comm);
