...
static __inline__ int a2a_sched_linear(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext, NBC_Schedule *schedule, void* sendbuf, int sendcount, MPI_Datatype sendtype, void* recvbuf, int recvcount, MPI_Datatype recvtype, NBC_Comminfo* comm);
static __inline__ int a2a_sched_pairwise(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext, NBC_Schedule *schedule, void* sendbuf, int sendcount, MPI_Datatype sendtype, void* recvbuf, int recvcount, MPI_Datatype recvtype, NBC_Comminfo* comm);
static __inline__ int a2a_sched_diss(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext, NBC_Schedule* schedule, void* sendbuf, int sendcount, MPI_Datatype sendtype, void* recvbuf, int recvcount, MPI_Datatype recvtype, NBC_Handle *handle, NBC_Comminfo* comm);
...
int NBC_Ialltoall(void* sendbuf, int sendcount, MPI_Datatype sendtype, void* recvbuf, int recvcount, MPI_Datatype recvtype, NBC_Comminfo* comm, NBC_Handle *handle)  {
  ...
  if (NBC_SUCCESS != res) { printf("MPI Error in NBC_Comm_rank() (%i)\n", res); return res; }
  res = NBC_Comm_size(comm, &p);
  if (NBC_SUCCESS != res) { printf("MPI Error in NBC_Comm_size() (%i)\n", res); return res; }
  ...
  if(!inplace) {
    ...
    res = NBC_Copy(sbuf, sendcount, sendtype, rbuf, recvcount, recvtype);
  ...
  if(alg == NBC_A2A_DISS) {
    ...
    if(NBC_Type_intrinsic(sendtype)) {
      ...
      res = MPI_Pack_size(sendcount, sendtype, MPI_COMM_WORLD, &datasize);
    ...
    if(NBC_Type_intrinsic(sendtype)) {
      ...
      /* non-contiguous - pack */
      res = MPI_Pack((char*)sendbuf+rank*sendcount*sndext, (p-rank)*sendcount, sendtype, handle->tmpbuf, (p-rank)*datasize, &pos, MPI_COMM_WORLD);
      if (MPI_SUCCESS != res) { printf("MPI Error in MPI_Pack() (%i)\n", res); return res; }
      if(rank != 0) {
        pos = 0;
        MPI_Pack(sendbuf, rank*sendcount, sendtype, (char*)handle->tmpbuf+datasize*(p-rank), rank*datasize, &pos, MPI_COMM_WORLD); 
        if (MPI_SUCCESS != res) { printf("MPI Error in MPI_Pack() (%i)\n", res); return res; }
      }
    }
  } else {
  ...
  if(found == NULL) {
    ...
    switch(alg) {
      case NBC_A2A_LINEAR: 
        ...
        break;
      case NBC_A2A_DISS:
        res = a2a_sched_diss(rank, p, sndext, rcvext, schedule, sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, handle, comm);
        break;
...
static __inline__ int a2a_sched_pairwise(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext, NBC_Schedule* schedule, void* sendbuf, int sendcount, MPI_Datatype sendtype, void* recvbuf, int recvcount, MPI_Datatype recvtype, NBC_Comminfo* comm) {
  ...
  for(r=1;r<p;r++) {
    ...
    res = NBC_Sched_recv(rbuf, false, recvcount, recvtype, rcvpeer, schedule, comm);
    if (NBC_OK != res) { printf("Error in NBC_Sched_recv() (%i)\n", res); return res; }
    sbuf = ((char *) sendbuf) + (sndpeer*sendcount*sndext);
    res = NBC_Sched_send(sbuf, false, sendcount, sendtype, sndpeer, schedule, comm);
...
static __inline__ int a2a_sched_linear(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext, NBC_Schedule* schedule, void* sendbuf, int sendcount, MPI_Datatype sendtype, void* recvbuf, int recvcount, MPI_Datatype recvtype, NBC_Comminfo* comm) {
  ...
  for(r=0;r<p;r++) {
    ...
    res = NBC_Sched_recv(rbuf, false, recvcount, recvtype, r, schedule, comm);
    if (NBC_OK != res) { printf("Error in NBC_Sched_recv() (%i)\n", res); return res; }
    sbuf = ((char *) sendbuf) + (r*sendcount*sndext);
    res = NBC_Sched_send(sbuf, false, sendcount, sendtype, r, schedule, comm);
    if (NBC_OK != res) { printf("Error in NBC_Sched_send() (%i)\n", res); return res; }
  }
...
static __inline__ int a2a_sched_diss(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext, NBC_Schedule* schedule, 
		void* sendbuf, int sendcount, MPI_Datatype sendtype, void* recvbuf, int recvcount, MPI_Datatype recvtype,
	       	NBC_Handle *handle, NBC_Comminfo *comm) {
  ...
  for(r = 1; r < p; r<<=1) {
    ...
    res = NBC_Sched_recv(rtmpbuf-(unsigned long)handle->tmpbuf, true, offset, MPI_BYTE, rpeer, schedule, comm);
    if (NBC_OK != res) { printf("Error in NBC_Sched_recv() (%i)\n", res); return res; }
 
    /*printf("[%i] sending %i bytes to host %i from sbuf %lu\n", rank, offset, speer, (unsigned long)stmpbuf);*/
    res = NBC_Sched_send(stmpbuf-(unsigned long)handle->tmpbuf, true, offset, MPI_BYTE, speer, schedule, comm);
    if (NBC_OK != res) { printf("Error in NBC_Sched_send() (%i)\n", res); return res; }
