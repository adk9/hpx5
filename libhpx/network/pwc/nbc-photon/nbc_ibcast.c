...
static __inline__ int bcast_sched_binomial(int rank, int p, int root, NBC_Schedule *schedule, void *buffer, 
		int count, MPI_Datatype datatype,NBC_Comminfo* comm);
static __inline__ int bcast_sched_linear(int rank, int p, int root, NBC_Schedule *schedule, void *buffer, 
		int count, MPI_Datatype datatype,NBC_Comminfo* comm);
static __inline__ int bcast_sched_chain(int rank, int p, int root, NBC_Schedule *schedule, void *buffer, int count, MPI_Datatype datatype, 
		int fragsize, int size, NBC_Comminfo* comm);
...
int NBC_Ibcast(void *buffer, int count, MPI_Datatype datatype, int root, NBC_Comminfo* comminfo, NBC_Handle* handle) {
  ...
  res = NBC_Init_handle(handle, comminfo);
  ...
  NBC_Comm_rank(comminfo, &rank);
  NBC_Comm_size(comminfo, &p);
  ...
  if(found == NULL) {
    ...
    switch(alg) {
      case NBC_BCAST_LINEAR:
        res = bcast_sched_linear(rank, p, root, schedule, buffer, count, datatype, comminfo);
        break;
      case NBC_BCAST_BINOMIAL:
        res = bcast_sched_binomial(rank, p, root, schedule, buffer, count, datatype, comminfo);
        break;
      case NBC_BCAST_CHAIN:
        res = bcast_sched_chain(rank, p, root, schedule, buffer, count, datatype, segsize, size, comminfo);
        break;
    }
...
static __inline__ int bcast_sched_binomial(int rank, int p, int root, NBC_Schedule *schedule, void *buffer,
  ...
  if(vrank != 0) {
    for(r=0; r<maxr; r++) {
      if((vrank >= (1<<r)) && (vrank < (1<<(r+1)))) {
        ...
        res = NBC_Sched_recv(buffer, false, count, datatype, peer, schedule, comm);
  ...
  for(r=0; r<maxr; r++) {
    if(((vrank + (1<<r) < p) && (vrank < (1<<r))) || (vrank == 0)) {
      VRANK2RANK(peer, vrank+(1<<r), root);
      res = NBC_Sched_send(buffer, false, count, datatype, peer, schedule, comm);
...
static __inline__ int bcast_sched_linear(int rank, int p, int root, NBC_Schedule *schedule, void *buffer, 
  ...
  if(rank == root) {
    ...
    /* recv msg from root */
    res = NBC_Sched_recv(buffer, false, count, datatype, root, schedule, comm);
    if (NBC_OK != res) { printf("Error in NBC_Sched_recv() (%i)\n", res); return res; }
  }
...
static __inline__ int bcast_sched_chain(int rank, int p, int root, NBC_Schedule *schedule, void *buffer,
	       	int count, MPI_Datatype datatype, int fragsize, int size, NBC_Comminfo* comm) {
  ...
  for(fragnum = 0; fragnum < numfrag; fragnum++) {
    ...
    if(vrank != 0) {
      res = NBC_Sched_recv(buf, false, thiscount, datatype, rpeer, schedule, comm);
      if (NBC_OK != res) { printf("Error in NBC_Sched_recv() (%i)\n", res); return res; }
      res = NBC_Sched_barrier(schedule);
    }
    ...
    if(vrank != p-1) {
      res = NBC_Sched_send(buf, false, thiscount, datatype, speer, schedule, comm);
      if (NBC_OK != res) { printf("Error in NBC_Sched_send() (%i)\n", res); return res; }
