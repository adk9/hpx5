...
uint64_t table_get(hpx_addr_t table, long i) {
  uint64_t val;
  size_t n = sizeof(val);
  hpx_addr_t there = hpx_addr_add(table, i*n, sizeof(uint64_t));
...
void table_set(hpx_addr_t table, long i, uint64_t val,
               hpx_addr_t lco) {
  hpx_addr_t there = hpx_addr_add(table, i*sizeof(uint64_t), sizeof(uint64_t));
...
static int _mover_action(guppie_config_t *cfg) {
  ...
  while (1) {
    ...
    hpx_addr_t there = hpx_addr_add(cfg->table, src * sizeof(uint64_t), sizeof(uint64_t));
...
void _main_action(guppie_config_t *cfg)
{
  ...
  fprintf(test_log, "nThreads = %d\n", hpx_get_num_ranks());
  fprintf(test_log, "Main table size = 2^%ld = %ld words\n", cfg->ltabsize, cfg->tabsize);
  fprintf(test_log, "Number of updates = %ld\n", cfg->nupdate);
  fflush(stdout);
  ...
  fprintf(test_log, "Initialization complete.\n");
  fflush(stdout);
  ...
  fprintf(test_log, "Completed updates.\n");
  fflush(stdout);
  ...
  fprintf(test_log, "init(c= %.4lf w= %.4lf) up(c= %.4lf w= %.4lf) up/sec= %.0lf\n",
  ...
  for (i=0; i<cfg->nupdate; i++) {
    temp = (temp << 1) ^ (((long) temp < 0) ? POLY : 0);
    there = hpx_addr_add(cfg->table, (temp & (cfg->tabsize-1))*sizeof(uint64_t), sizeof(uint64_t));
