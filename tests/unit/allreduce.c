...
static int _test_bcast_handler(hpx_addr_t allreduce, hpx_addr_t sum, hpx_action_t leaf) {
  int row = HPX_LOCALITIES * HPX_LOCALITY_ID;
  for (int j = 0; j < N; ++j) {
    int i = row + j;
    // &sum is passed explicitly instead of as a continuation for the _join_leaf
    CHECK( hpx_call(HPX_HERE, leaf, HPX_NULL, &allreduce, &i, &j, &sum) );
  }
  return HPX_SUCCESS;
}
static HPX_ACTION(HPX_DEFAULT, 0, _test_bcast, _test_bcast_handler, HPX_ADDR, HPX_ADDR, HPX_ACTION_T);

/// A utility that tests a certain leaf function through N iterations.
///
/// This makes sure that the allreduced value is the expected value, i.e., that
/// for each of N iterations we get N copies of the reduced value,
/// N*(N-1)/2 (except that N starts at 0, so it is (N-1)*N/2).
static int _test(hpx_action_t leaf) {
  int r;
  int n = N * HPX_LOCALITIES;
  hpx_addr_t allreduce = hpx_lco_allreduce_new(n, n, sizeof(int), _init, _sum);
  hpx_addr_t sum = hpx_lco_reduce_new(n * N, sizeof(int), _init, _sum);
  for (int i = 0; i < N; ++i) {
    CHECK( hpx_bcast(_test_bcast, HPX_NULL, HPX_NULL, &allreduce, &sum, &leaf) );
  }
  CHECK( hpx_lco_get(sum, sizeof(r), &r) );
  test_assert(r == (N - 1) * N * N * N / 2);
...
static int _set_get_leaf_handler(hpx_addr_t allreduce, int i, int j, hpx_addr_t sum) {
  int r;
...
static HPX_ACTION(HPX_DEFAULT, 0, _set_get_leaf, _set_get_leaf_handler, HPX_ADDR, HPX_INT, HPX_INT, HPX_ADDR);

/// Use a join operation in the allreduce leaf.
static int _join_leaf_handler(hpx_addr_t allreduce, int i, int j, hpx_addr_t sum) {
  CHECK( hpx_lco_allreduce_join(allreduce, i, sizeof(j), &j, hpx_lco_set_action, sum) );
}
static HPX_ACTION(HPX_DEFAULT, 0, _join_leaf, _join_leaf_handler, HPX_ADDR, HPX_INT, HPX_INT, HPX_ADDR);

/// Use the join_async operation in the allreduce leaf.
static int _join_async_leaf_handler(hpx_addr_t allreduce, int i, int j, hpx_addr_t sum) {
  int r;
...
static HPX_ACTION(HPX_DEFAULT, 0, _join_async_leaf, _join_async_leaf_handler, HPX_ADDR, HPX_INT, HPX_INT, HPX_ADDR);

/// Use the join_sync operation in the allreduce leaf.
static int _join_sync_leaf_handler(hpx_addr_t allreduce, int i, int j, hpx_addr_t sum) {
  int r;
  CHECK( hpx_lco_allreduce_join_sync(allreduce, i, sizeof(j), &j, &r) );
  hpx_call_cc(sum, hpx_lco_set_action, NULL, NULL, &r, sizeof(r));
}
static HPX_ACTION(HPX_DEFAULT, 0, _join_sync_leaf, _join_sync_leaf_handler, HPX_ADDR, HPX_INT, HPX_INT, HPX_ADDR);

/// Spawn the set-get test.
static int _test_allreduce_set_get_handler(void) {
  return _test(_set_get_leaf);
}
static HPX_ACTION(HPX_DEFAULT, 0, _test_allreduce_set_get, _test_allreduce_set_get_handler);

/// Spawn the join test.
static int _test_allreduce_join_handler(void) {
  return _test(_join_leaf);
}
static HPX_ACTION(HPX_DEFAULT, 0, _test_allreduce_join, _test_allreduce_join_handler);

/// Spawn the join async test.
static int _test_allreduce_join_async_handler(void) {
  return _test(_join_async_leaf);
}
static HPX_ACTION(HPX_DEFAULT, 0, _test_allreduce_join_async, _test_allreduce_join_async_handler);

/// Spawn the join sync test.
static int _test_allreduce_join_sync_handler(void) {
  return _test(_join_sync_leaf);
}
static HPX_ACTION(HPX_DEFAULT, 0, _test_allreduce_join_sync, _test_allreduce_join_sync_handler);
