...
int __verbs_setup_ri_ledgers(ProcessInfo *verbs_processes, char *buf, int num_entries) {
	...
	for(i = 0; i < _photon_nproc + _photon_forwarder; i++) {
		...
		// allocate the ledger
		verbs_processes[i].local_rcv_info_ledger = photon_ri_ledger_create_reuse((photonRILedgerEntry) (buf + ledger_size * i), num_entries);
		...
		verbs_processes[i].remote_rcv_info_ledger = photon_ri_ledger_create_reuse((photonRILedgerEntry) (buf + ledger_size * (_photon_nproc+_photon_forwarder) + ledger_size * i), num_entries);
	...
	for(i = 0; i < _photon_nproc+_photon_forwarder; i++) {
		...
		// allocate the ledger
		verbs_processes[i].local_snd_info_ledger = photon_ri_ledger_create_reuse((photonRILedgerEntry) (buf + offset + ledger_size * i), num_entries);
		...
		verbs_processes[i].remote_snd_info_ledger = photon_ri_ledger_create_reuse((photonRILedgerEntry) (buf + offset + ledger_size * (_photon_nproc+_photon_forwarder) + ledger_size * i), num_entries);
...
int __verbs_setup_FIN_ledger(ProcessInfo *verbs_processes, char *buf, int num_entries) {
	...
	for(i = 0; i < (_photon_nproc+_photon_forwarder); i++) {
		...
		verbs_processes[i].local_FIN_ledger = photon_rdma_FIN_ledger_create_reuse((photonFINLedgerEntry) (buf + ledger_size * i), num_entries);
		...
		verbs_processes[i].remote_FIN_ledger = photon_rdma_FIN_ledger_create_reuse((photonFINLedgerEntry) (buf + ledger_size * (_photon_nproc+_photon_forwarder) + ledger_size * i), num_entries);
