...
int _photon_put_with_completion(int proc, void *ptr, uint64_t size, void *rptr,
  ...
  if (nentries > 0) {
    req = photon_setup_request_direct(&rbuf, proc, nentries);
    if (req == NULL) {
...
int _photon_get_with_completion(int proc, void *ptr, uint64_t size, void *rptr,
  ...
  rc = __photon_backend->rdma_get(proc, (uintptr_t)ptr, (uintptr_t)rptr, size, &(db->buf), &rbuf, cookie, 0);
  if (rc != PHOTON_OK) {
