...
int photon_pwc_add_req(photonRequest req) {
  dbg_trace("enqueing request: 0x%016lx", req->id);
...
static int photon_pwc_process_queued_gwc(int proc, photonRequestTable rt) {
  ...
  req = sync_ms_queue_dequeue(rt->gwc_q);
  assert(req);

  dbg_info("Trying to process queued GWC");
  
  rc = __photon_backend->rdma_get(proc, (uintptr_t)req->local_info.buf.addr,
				  (uintptr_t)req->remote_info.buf.addr, req->size,
				  &(req->local_info.buf), &(req->remote_info.buf),
  ...
  dbg_info("sending GWC: 0x%016lx", req->id);
...
static int photon_pwc_process_queued_pwc(int proc, photonRequestTable rt) {
  ...
  rc = photon_pwc_test_ledger(proc, &offset);
  if (rc == PHOTON_OK) {
    req = sync_ms_queue_dequeue(rt->pwc_q);
    assert(req);
...
int _photon_put_with_completion(int proc, void *ptr, uint64_t size, void *rptr,
 ...
 queue_exit:
  sync_ms_queue_enqueue(rt->pwc_q, req);
  sync_fadd(&rt->pcount, 1, SYNC_RELAXED);
  //dbg_info("Enqueued request: 0x%016lx", req->id);
  return PHOTON_OK;
...
int _photon_get_with_completion(int proc, void *ptr, uint64_t size, void *rptr,
  ...
  // process any queued gwc requests for this peer first
  rc = photon_pwc_process_queued_gwc(proc, rt);
  if (rc == PHOTON_OK) {
    goto queue_exit;
  }

  // process any queued pwc requests for this peer first
  rc = photon_pwc_process_queued_pwc(proc, rt);
  if (rc == PHOTON_OK) {
    goto queue_exit;
  }
 ...
 queue_exit:
  sync_ms_queue_enqueue(rt->gwc_q, req);
  sync_fadd(&rt->gcount, 1, SYNC_RELAXED);
  dbg_info("Enqueued GWC request: 0x%016lx", req->id);
  return PHOTON_OK;
  ...
  *flag = 0;
  ...
  if (flags & PHOTON_PROBE_EVQ) {
	...
	*request = req->local_info.id;
    ...
    if (rc == PHOTON_EVENT_ERROR) {
      ...
      cookie = NULL_REQUEST;
      // no event so process any queued requests
      for (i=start; i<end; i++) {
	...
	photon_free_request(req);
	cleared++;
	dbg_trace("%d requests remaining", photon_count_request(PHOTON_ANY_SOURCE));
	dbg_trace("pwc=%d, gwc=%d, total=%d, cleared=%d", pwc, gwc, total, cleared);
	goto exit;
      }
      else {
  ...
  if ((cookie == NULL_REQUEST) && (flags & PHOTON_PROBE_LEDGER)) {
	...
	*flag = 1;
	...
	dbg_trace("Popped ledger event with id: 0x%016lx (%lu), addr=0x%016lx", *request, *request, (uintptr_t)entry_iter);
	dbg_trace("%d requests remaining", photon_count_request(PHOTON_ANY_SOURCE));
	dbg_trace("pwc=%d, gwc=%d, total=%d, cleared=%d", pwc, gwc, total, cleared);
	goto exit;
      }
